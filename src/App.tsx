...
import { Text, useSize, Input, View, Frame, useWordWrap } from "react-curse";
...
const App: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    ...
    { content: "What about props? How do I pass data to components?", role: "user" },
    {
      content:
    ...
    { content: "Can you show me how to use state in a component?", role: "user" },
    {
      content:
    ...
    { content: "What's the difference between functional and class components?", role: "user" },
    {
      content:
    ...
    { content: "What are React keys and why are they important?", role: "user" },
    {
      content:
    ...
    <>
      <View height={height - 6} width="100%" focus={false}>
        {messages.map((msg, index) => {
          ...
          return (
            <Fragment key={index}>
              <Frame x={alignX} width={frameWidth} height={frameHeight} type="rounded" color={color}>
                <Text color={color}>
                  {wrappedContent}
                </Text>
              </Frame>
